<?php
	...
	if (!empty($_COOKIE['BORROWERID'])) {
		...
		$select->execute(array($_COOKIE['BORROWERID']));
		$row = $select->fetch();
		if($row){
			...
			$ex = $update->execute(array($photo, $borrower_title, $borrower_firstname, $borrower_lastname, $borrower_gender, $borrower_country, $borrower_city, $borrower_address, $borrower_email, $borrower_phone, $borrower_dateofbirth, $borrower_working_status, $borrower_employer_name, $borrower_employer_phone, $borrower_employer_address, $borrower_business, $borrower_business_type, $borrower_business_address, $_COOKIE['BORROWERID'], $BORROWERID));
			if($ex){
