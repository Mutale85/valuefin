<?php
	...
	if (isset($_POST['remarks'])) {
		...
		if ($remarks_id !== "") {
			# update
			$up = $connect->prepare("UPDATE reports_arrear_loans SET remarks = ? WHERE id = ? AND loan_number = ? ");
			$up->execute(array($remarks, $remarks_id, $loan_number));
			echo 'Remarks Updated';
		}else{
			$date_added = date("Y-m-d");
			$sql = $connect->prepare("INSERT INTO `reports_arrear_loans`(`branch_id`, `parent_id`, `loan_number`, `remarks`, `borrower_id`, `user_id`, `date_added`) VALUES(?, ?, ?, ?, ?, ?, ?) ");
	...
	if (isset($_POST['remarksEditId'])) {
		...
		$query = $connect->prepare("SELECT * FROM reports_arrear_loans WHERE id = ? AND parent_id = ? ");
	...
	if (isset($_POST['deleteEditId'])) {
		...
		// $delete = $connect->prepare("DELETE FROM reports_arrear_loans WHERE id = ? AND borrower_id = ? ");
		$delete = $connect->prepare("UPDATE reports_arrear_loans SET display = '0' WHERE id = ? AND borrower_id = ? ");
