...
<html>
<head>
	<title>View Group Borrowers</title>
...
<?php
	if(isset($_GET['group_id'])){
		$query = $connect->prepare("SELECT * FROM group_borrowers WHERE group_id = ?");
		$query->execute(array($_GET['group_id']));
		$row = $query->fetch();
		$group_name = "";
		if($row){
			$group_id =  $row['group_id'];
			$group_name =  $row['group_name'];
			$names =  $row['borrowers_id'];
			$group_leader_id =  $row['group_leader_id'];
			$collectors_name =  $row['collectors_name'];
			$description =  $row['description'];
		}

		$sql = $connect->prepare("SELECT * FROM `loans` WHERE loan_number = ?");
		...
		<div class="content-wrapper">
			<section class="content bg-light mt-5">
      			<div class="container mt-5 mb-5">
      				<div class="row mt-5 pb-2 d-flex justify-content-between">
      					...
      					<div class="col-md-4 mt-4 ">
  							<div class="card mb-4  bg-success">
  								<div class="card-body text-center">
  									<h4>Group Leader</h4>
  									<?php echo getBorrowerFullNames($connect, $group_leader_id) ?>
  								</div>
  							</div>
      					</div>
      					<div class="col-md-4 mt-4 ">
  							<div class="card mb-4  bg-primary">
  								<div class="card-body text-center">
  									<h4>Collector</h4>
  									<?php echo $collectors_name ?>
  								</div>
  							</div>
      					</div>
      					<div class="col-md-12 mt-4 mb-5">
      						<p class="fs-5"><b>Description:</b> <?php echo $description ?></p>
      					</div>
      				</div>
      			</div>
      			<!-- borrower form -->
      						...
      						<div class="card card-primary mb-5">
      							...
      							<div class="card-body box-profile">
      								<div class="table table-responsive">
			      						<table id="myTable" class="cell-table table table-sm" style="width:100%">
									        <thead>
									            <tr>
									            	<th>Serial #</th>
									                <th>Group Member</th>
									        ...
									        <tbody>
									        <?php
									        	$query = $connect->prepare("SELECT * FROM group_borrower_members WHERE group_unique_id = ? AND parent_id = ?");
									        	$query->execute(array($_GET['group_id'], $_SESSION['parent_id']));
									        	$i = 1;
												foreach ($query->fetchAll() as $row) {
													$borrower_id = $row['borrower_id'];
													$sql = $connect->prepare("SELECT * FROM borrowers WHERE id = ? AND parent_id = ?");
													$sql->execute(array($borrower_id, $_SESSION['parent_id']));
													$rows = $sql->fetch();
													$fname = $lname = $card_id = "";
													if ($rows) {
														$card_id = $rows['borrower_ID'];
														$fname = $rows['borrower_firstname'];
														$lname = $rows['borrower_lastname'];
													}
											?>
												<tr>
													<td><?php echo $i++?></td>
													...
													<td><?php echo countLoans($connect, $card_id, $_SESSION['parent_id'])?></td>
													<td>
														<a href="borrowers/view_borrower_details?borrower_id=<?php echo $card_id ?>" class="text-primary">See Member</a>
													</td>
												</tr>
											<?php
												}
      								...
      								<div class="table table-responsive">
			      						<table id="loansTable" class="cell-border table table-sm" style="width:100%">
											...
											<tbody>
		      								<?php
		      									// 
		      									$query = $connect->prepare("SELECT * FROM loans WHERE branch_id = ? AND parent_id = ? AND loan_number = ? ");
												$query->execute(array($BRANCHID,  $_SESSION['parent_id'], $_GET['group_id']));
												$results = $query->fetchAll();
												if ($query->rowCount() > 0) {
													$group_leader_id  = $this_loan_number = "";
													foreach ($results as $row) {
														...
														$sql->execute(array($id , $this_loan_number, $borrower_id));
														if ($sql->rowCount() > 0) {
															$rows = $sql->fetch();
															if ($rows) {
																extract($rows);
													...
													<tr>
														...
														<td>
															<a href="<?php echo getIDNumberForGroupLeader($connect, $_GET['group_leader_id'], $_GET['parent_id']) ?>" data-loan_id="<?php echo getIDNumberForGroupLeader($connect, $_GET['group_leader_id'], $_GET['parent_id']) ?>" data-borrower_id="<?php echo $_GET['borrower_id']?>" class="btn btn-outline-primary btn-xs takeAction" id="<?php echo $_GET['group_leader_id'] ?>">Action <i class="bi bi-subtract"></i></a>		
														</td>
													</tr>
												<?php
													}
				              ...
				              <div class="card-body">
				                <div class="tab-content" id="custom-tabs-two-tabContent">
				                     	...
				                     	<div class="table table-responsive">
		                     				<table class="cell-border table table-sm" id="paymentsTable" width="100%">
				                     			...
				                     			<?php
				                     				...
				                     				$sql->execute(array($ID , $_GET['group_id'], getIDNumberForGroupLeader($connect, $_GET['group_leader_id'], $_GET['parent_id'])));
				                     				if ($sql->rowCount() > 0) {
				                     					...
				                     					foreach ($sql->fetchAll() as $rows) {
				                     						...
				                     						<tr>
					                     						...
					                     						<td>
					                     							<a href="loans/payment_receitp?loan_number=<?php echo $loan_number?>&branch_id=<?php echo $BRANCHID?>&parent_id=<?php echo $parent_id?>&borrower_id=<?php echo getIDNumberForGroupLeader($connect, $_GET['group_leader_id'], $_GET['parent_id'])?>" class="pdf"> <i class="bi bi-printer"></i> View</a>
					                     						</td>
					                     					</tr>
				                     					<?php
				                     					}	
				                  	...
				                  	<div class="tab-pane fade" id="custom-tabs-two-settings" role="tabpanel" aria-labelledby="custom-tabs-two-settings-tab">
				                  		<div class="mb-3">
				                  			<a href="collaterals/collateral?loan_number=<?php echo $_GET['group_id']?>&borrower_id=<?php echo getIDNumberForGroupLeader($connect, $_GET['group_leader_id'], $_GET['parent_id'])?>" class="btn btn-primary shadow">Add Collateral</a>
				                  		</div>
				                  		<div class="table table-responsive">
				                  		...
				                  		<?php
				                  			$sqlC = $connect->prepare("SELECT * FROM `collaterals` WHERE loan_number = ? AND parent_id = ? ");
											$sqlC->execute(array($_GET['group_id'], $_GET['parent_id']));
											if ($sqlC->rowCount() > 0) {
												$res = $sqlC->fetchAll();
												foreach ($res as $row) {
													extract($row);
							...
							<div class="modal-body">
								<form method="post" id="loanActionForm">
									<div class="form-group mb-3">
										...
										<input type="hidden" name="loan_number" id="loan_number" value="<?php echo $_GET['group_id']?>">
									</div>
							...
							<div class="modal-body">
								<form method="post" id="loanPaymentForm">
									...
									<div class="form-group mb-3">
										...
										<input type="hidden" name="loan_id" id="loan_id" value="<?php echo $ID?>">
										<input type="hidden" name="loan_number" id="loan_number" value="<?php echo $_GET['group_id'] ?>">
										<input type="hidden" name="borrower_id" id="borrower_id" value="<?php echo getIDNumberForGroupLeader($connect, $_GET['group_leader_id'], $_GET['parent_id'])?>">
									</div>
