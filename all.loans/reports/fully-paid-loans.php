...
<html>
<head>
	<title>Fully Repaid Loans Reports</title>
      					...
      					<div class="col-md-12">
      						<div class="card card-primary">
      							<div class="card-header">
      								<h4 class="card-title"> Fully Paid Loans </h4>
      							</div>
      							<div class="card-body">
      										...
      										<tbody class="text-dark">
			      								<?php
			      									if (isset($_GET['from_period']) AND isset($_GET['to_period'])) {
			      										...
			      										$query = $connect->prepare("SELECT loans_table.*, loan_payments.* FROM loans_table JOIN loan_payments ON loans_table.loan_number = loan_payments.loan_number WHERE loans_table.release_date BETWEEN ? AND ? AND loans_table.branch_id = ? AND loans_table.parent_id = ? AND loans_table.loan_status = 'Released' AND loan_payments.balance = '0.00' ");
			      										$query->execute(array($_GET['from_period'], $_GET['to_period'], $BRANCHID,  $_SESSION['parent_id']));
			      									}else{
			      										$query = $connect->prepare("SELECT loans_table.*, loan_payments.* FROM loans_table JOIN loan_payments ON loans_table.loan_number = loan_payments.loan_number WHERE loans_table.branch_id = ? AND loans_table.parent_id = ? AND loans_table.loan_status = 'Released' AND loan_payments.balance = '0.00' ");
													...
													if ($query->rowCount() > 0) {
														...
														foreach ($query->fetchAll() as $row) {
															extract($row);
															$update = $connect->prepare("UPDATE loans_table SET loan_status = 'Completed' WHERE loan_number = ? AND borrower_id = ? ");
															$update->execute(array($loan_number, $borrower_id));
														?>
														...
														<tr>
															...
															<td>
																...
																<?php
																	$query = $connect->prepare("SELECT * FROM loan_payments WHERE loan_number = ? AND borrower_id = ? ");
																	$query->execute(array($loan_number, $borrower_id));
																	if ($query->rowCount() > 0) {?>
																	...
																	?>
																			...
																			<tr>
																				<td>Date:</td>
																				<td>
																					<small> <?php echo date("l, jS \of F Y", strtotime($paid_date))?></small>
																				</td>
																			</tr>
																	<?php
																		}
																		$query = $connect->prepare("SELECT SUM(amount) AS total_paid FROM loan_payments WHERE loan_number = ? AND borrower_id = ? ");
																		$query->execute(array($loan_number, $borrower_id));
																		$rows = $query->fetch();
																	?>
																			<tr>
																				<th>Total Paid</th>
																				<th><?php echo $currency?> <?php echo $rows['total_paid']?></th>
																			</tr>
																		</table>
																<?php	}
																?>
															...
															<td>
																<?php
																	$query = $connect->prepare("SELECT * FROM reports_fully_paid_loans WHERE loan_number = ? AND display = '1' ");
