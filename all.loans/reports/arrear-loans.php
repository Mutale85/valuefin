<?php 
  	...
  	$_1_ago = date('Y-m-d',strtotime("-1 days"));
  	$_2_ago = date('Y-m-d',strtotime("-2 days"));
  	$_3_ago = date('Y-m-d',strtotime("-3 days"));
  	$_4_ago = date('Y-m-d',strtotime("-4 days"));
  	$_5_ago = date('Y-m-d',strtotime("-5 days"));  
?>
<!DOCTYPE html>
<html>
<head>
	<title>Arrear Loans Reports</title>
      					...
      					<div class="col-md-12">
      						<div class="card card-primary">
      							<div class="card-header">
      								<h4 class="card-title"> Arrear Loan for </h4>
      							</div>
      							<div class="card-body">
      										...
      										<tbody class="text-dark">
			      								<?php
			      									if (isset($_GET['from_period']) AND isset($_GET['to_period'])) {
			      										...
			      										// $query = $connect->prepare("SELECT * FROM loan_schedules WHERE `date_due` BETWEEN ? AND ? AND branch_id = ? AND parent_id = ? AND loan_status = 'Released' ");
			      										$query = $connect->prepare("SELECT loans_table.*, loan_schedules.* FROM loans_table JOIN loan_schedules ON loans_table.borrower_id = loan_schedules.borrower_id WHERE loan_schedules.date_due = BETWEEN ? AND ? AND loans_table.branch_id = ? AND loans_table.parent_id = ? AND loans_table.loan_status = 'Released' ");

			      										$query->execute(array($_GET['from_period'], $_GET['to_period'], $BRANCHID,  $_SESSION['parent_id']));
			      									}else{
			      										
			      										
			      										$query = $connect->prepare("SELECT loans_table.*, loan_schedules.* FROM loans_table JOIN loan_schedules ON loans_table.borrower_id = loan_schedules.borrower_id WHERE loans_table.branch_id = ? AND loans_table.parent_id = ? AND loans_table.loan_status = 'Released' AND loan_schedules.date_due = ? OR loan_schedules.date_due = ?  OR loan_schedules.date_due = ? OR loan_schedules.date_due = ? OR loan_schedules.date_due = ?   ");
					  									$today = date("Y-m-d");
					  									$query->execute(array($BRANCHID, $_SESSION['parent_id'], $yesterday, $_2_ago, $_3_ago, $_4_ago, $_5_ago));
			      									}
													...
													if ($query->rowCount() > 0) {
														...
														<script>
															$("#loanRemarksForm_<?php echo $borrower_id?>").submit(function(e){
																...
																$.ajax({
																	url:'reports/actionsReports_Arrears?<?php echo time()?>',
																	method:"post",
																	data:data,
																	cache : false,
												    				processData: false,
	...
	<script>
		$(document).ready( function () {
		    ...
		    $(document).on("click", ".editRemarks", function(e){
		    	...
		    	$("#remarksModal_"+borrower_id).modal("show");
		    	$.ajax({
					url:'reports/actionsReports_Arrears',
					method:"post",
